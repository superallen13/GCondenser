...
from graph_condenser.models.components.traj_gnn import TrajCL
...
class TrajectoryBuffer:
    def __init__(
        ...
        self.traj_dir = os.path.join(traj_dir, f"{model}_dropout_{dropout}_lr_{lr}_wd_{wd}_epochs_{epochs}_T_{T}_lambda_{lamb}_scheduler_{scheduler}_interval_{param_save_interval}")
        self.device = device
    ...
    def get(self, epoch, rand_start=False):
        if rand_start:
            find_epoch = random.randint(0, epoch)
        else:
            find_epoch = epoch
        idx = find_epoch // self.param_save_interval
        return find_epoch, self.expert_trajectory[idx]
...
def main(args):
    path = os.path.join(
        args.traj_dir,
        args.dataset,
        f"{args.backbone}_dropout_{args.dropout}_lr_{args.lr}_wd_{args.wd}_epochs_{args.epochs}_T_{args.T}_lambda_{args.lamb}_scheduler_{args.scheduler}_interval_{args.param_interval}",
    )
...
if __name__ == "__main__":
    """ Usage: python graph_condenser/models/components/trajectories_cl.py --lr 0.001 --epochs 1000 --dataset citeseer --num_experts=200 --wd 5e-4 --lamb 0.8 --T 250 --scheduler root
    """
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, default="./data")
    parser.add_argument("--dataset", type=str, default="citeseer")
    parser.add_argument("--observe_mode", type=str, default="transductive")
    parser.add_argument("--traj_dir", type=str, default="./data/traj_cl")
    ...
    parser.add_argument("--lamb", type=float, default=0.75)
