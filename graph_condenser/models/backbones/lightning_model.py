...
class LightningModel(pl.LightningModule):
    ...
    def training_step(self, data, batch_idx):
        y_hat = self(data.ndata["feat"])[data.ndata["train_mask"]]
    ...
    def validation_step(self, data, batch_idx):
        y_hat = self(data.ndata["feat"])[data.ndata["val_mask"]]
    ...
    def test_step(self, data, batch_idx):
        y_hat = self(data.ndata["feat"])[data.ndata["test_mask"]]
    ...
    def configure_optimizers(self):
        return torch.optim.Adam(
            self.backbone.parameters(), lr=self.hparams.lr, weight_decay=self.hparams.wd
        )
